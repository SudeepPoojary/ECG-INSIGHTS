...
class ECG:
    ...
    def SignalExtraction_Scaling(self, Leads):
        for x, y in enumerate(Leads[:len(Leads) - 1]):
            ...
            csv_filename = os.path.join(csv_folder, 'Scaled_1DLead_{lead_no}.csv'.format(lead_no=lead_no + 1))
            if os.path.isfile(csv_filename):
                Normalized_Scaled.to_csv(csv_filename, mode='a', index=False)
            else:
                Normalized_Scaled.to_csv(csv_filename, index=False)

    def CombineConvert1Dsignal(self):
        test_final = pd.read_csv('csv_files/Scaled_1DLead_1.csv')
        location = os.getcwd()

        # loop over all the 11 remaining leads and combine as one dataset using pandas concat
        for files in natsorted(os.listdir('csv_files')):
            if files.endswith(".csv"):
                if files != 'Scaled_1DLead_1.csv':
                    df = pd.read_csv(os.path.join('csv_files', files))
